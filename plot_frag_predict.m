...
function plot_frag_predict(shuffled_link,nth_wkday,road_namestr)
	...
	for nth_link = 1:length(shuffled_link)
		...
		for nth_index=1:length(index_str)
			eval(['link_',index_str{nth_index},'(:,nth_link) = vertcat(shuffled_link(nth_link).params(:,nth_wkday).',index_str{nth_index},');'])
		end
			% link_lowpredict(:,nth_link) = vertcat(shuffled_link(nth_link).params(:,nth_wkday).lowpredict);
	...
	for nth_index=1:length(index_str)
		...
		caxis([0,1]);
		colorbar;
		xlabel('nth_link','Interpreter', 'none');
