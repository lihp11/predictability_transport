...
function plot_road_predict(road_params,road_namestr)
	...
	for nth_index=1:length(index_str)
		if ~(nth_index==1 || nth_index==2)
			eval(['road_',index_str{nth_index},' = road_',index_str{nth_index},'./max(max(road_',index_str{nth_index},'));'])
			% road_lowpredict = road_lowpredict./max(max(road_lowpredict));
		end
        ...
        saveas(gcf,['.\pic_out_min\',road_namestr,'_sumtt_',index_str{nth_index},'.png']);
    ...
    saveas(gcf,['.\pic_out_min\',road_namestr,'_index_compare','.png']);
    hold off;
end
